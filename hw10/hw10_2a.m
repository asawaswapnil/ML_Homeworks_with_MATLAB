...
%bagging
 [test_y_5bag] = Bag_classifier(tr_x,tr_y,test_x,['[@SVML_base,',num2str(5),',[]]']);
[test_y_base] = Bag_classifier(tr_x,tr_y,test_x,'[@SVML_base,1,[]]');
...
for i=1:10
    for j=1:20
        [test_y_bag(:,i)] = Bag_classifier(tr_x,tr_y,test_x,['[@SVML_base,',num2str(i),',[]]']);
...
for i=1:10
    for j=1:20
        [test_y_boost(:,i)] = Boost_classifier(tr_x,tr_y,test_x,['[@SVML_base,',num2str(i),',[]]']);
...
%bagging
 [test_y_5bag] = Bag_classifier(tr_x,tr_y,test_x,['[@SVML_base,',num2str(5),',[]]']);
[test_y_base] = Bag_classifier(tr_x,tr_y,test_x,'[@SVML_base,1,[]]');
Error_5_tr=sum(abs(test_y_5bag-test_orig_y))/size(test_x,1)
Error_base_tr=sum(abs(test_y_base-test_orig_y))/size(test_x,1)
...
for i=1:10
    for j=1:20
        [test_y_bag(:,i)] = Bag_classifier(tr_x,tr_y,tr_x,['[@SVML_base,',num2str(i),',[]]']);
...
for i=1:10
    for j=1:20
        [test_y_boost(:,i)] = Boost_classifier(tr_x,tr_y,tr_x,['[@SVML_base,',num2str(i),',[]]']);
